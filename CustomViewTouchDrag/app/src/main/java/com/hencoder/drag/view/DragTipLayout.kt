...
import kotlinx.android.synthetic.main.drag_tip.view.*

/**
 * 使用 OnDragListener
 * 长按后拖起半透明图像的控件，并且当手指把这个图像放在界面中某一个位置的时候松手，界面中会用 Toast 显示出相应的效果表示「落在这里了」
 */
class DragTipLayout(context: Context, attrs: AttributeSet?) : ConstraintLayout(context, attrs) {
    ...
    inner class CollectListener : OnDragListener {
        override fun onDrag(v: View, event: DragEvent): Boolean {
            Log.e("TAG", "OnDragListener event.action = ${event.action}")
            when (event.action) {
                //只需要落下事件, 这个 v 是底部的 LinearLayout
                DragEvent.ACTION_DROP ->{
                    Log.e("TAG", "OnDragListener DragEvent.ACTION_DROP Toast")
                    Toast.makeText(this@DragTipLayout.context, "落在这里了", Toast.LENGTH_SHORT).show()
                }
