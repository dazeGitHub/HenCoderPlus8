...
class NestedScalableImageView(context: Context?, attrs: AttributeSet?) : View(context, attrs), NestedScrollingChild3 {
    ...
    inner class HenGestureListener : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onScroll(
            ...
            if (big) {
                offsetX -= distanceX
                offsetY -= distanceY
                //unconsumed 就是还剩多少偏移量没用
                val unconsumed = fixOffsets()
                if (unconsumed != 0) {
                    //询问父 View 是否滑动
                    //调用 NestedScrollingChildHelper 类型的对象的 dispatchNestedScroll() 方法将没有用完的偏移量传递给父 View
